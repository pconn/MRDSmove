#' Negative log likelihood function for MRDS double observer data subject to movement and measurement error for models estimating observer dependence
...
#' @param dep.formula Formula object for implementing observer dependence as described in discussion of MacKenzie and Clement 2016.  Note that the
#'        formula must use "det_other" in some fashion; for instance, an interaction between "det_other" and "distance" (i.e. ~0+det_other:distance) can be used to implement point indendence.
...
#' @author Paul B. Conn
MRDSmove_IntLik_ObsDep <- function(Par,Data,p.formula,dep.formula,Bin.widths,Obs.bins,Move.fix=NULL,gaussian.move=FALSE,gaussian.meas=FALSE){
  Cur.par = Par
  n.par = length(Par)
  ...
  Bin.probs = Bin.widths/sum(Bin.widths)
  ...
  P.dot = array(0,dim=c(n.bins,n.bins,n.hists))
  ...
  for(ibin1 in 1:n.bins){  #observer 1 latent distance
    for(ibin2 in 1:n.bins){  #observer 2 latent distance
      ...
      # P.hist[Which.11] = P.hist[Which.11]+P.dist[ibin1,ibin2,Which.11]*P[ibin1,ibin2,1,Which.11]*P[ibin1,ibin2,2,Which.11]*Meas[ibin1,Obs.dists[1,Which.11]]*Meas[ibin2,Obs.dists[2,Which.11]]/P.dot[ibin1,ibin2,Which.11]
      # #components of Pr(10 hists)
      # P.hist[Which.10] = P.hist[Which.10]+P.dist[ibin1,ibin2,Which.10]*P[ibin1,ibin2,1,Which.10]*(1-P[ibin1,ibin2,2,Which.10]*sum(Meas[ibin2,Obs.bins]))*Meas[ibin1,Obs.dists[1,Which.10]]/P.dot[ibin1,ibin2,Which.10]
      # #components of Pr(01 hists)
      # P.hist[Which.01] = P.hist[Which.01]+P.dist[ibin1,ibin2,Which.01]*P[ibin1,ibin2,2,Which.01]*(1-P[ibin1,ibin2,1,Which.01]*sum(Meas[ibin1,Obs.bins]))*Meas[ibin2,Obs.dists[2,Which.01]]/P.dot[ibin1,ibin2,Which.01]
      #probability of Pr(11 hists)
      P.hist[Which.11] = P.hist[Which.11]+P.dist[ibin1,ibin2,Which.11]*P[ibin1,ibin2,1,Which.11]*P[ibin1,ibin2,2,Which.11]*Meas[ibin1,Obs.dists[1,Which.11]]*Meas[ibin2,Obs.dists[2,Which.11]]/P.dot[ibin1,ibin2,Which.11]
      #components of Pr(10 hists)
      P.hist[Which.10] = P.hist[Which.10]+P.dist[ibin1,ibin2,Which.10]*(P.yes[ibin1,ibin2,1,Which.10]*P.yes[ibin1,ibin2,2,Which.10]*Meas[ibin1,Obs.dists[1,Which.10]]*(1-Pin[ibin2])+P.no[ibin1,ibin2,1,Which.10]*(1-P.yes[ibin1,ibin2,2,Which.10])*Meas[ibin1,Obs.dists[1,Which.10]])/P.dot[ibin1,ibin2,Which.10]
      #components of Pr(01 hists)
